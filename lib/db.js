...
function init() {
  ...
  initPromise = (async () => {
    ...
    `;
    // Notification preferences per merchant (which channels get order alerts).
    await sql`
      ALTER TABLE merchants
    ...
    await sql`
      ALTER TABLE merchants
      ADD COLUMN IF NOT EXISTS notify_channels JSONB NOT NULL DEFAULT '["dashboard"]'::jsonb
    `;
    // Orders placed through the storefront. status:
    //   pending → new, unseen
    //   confirmed → merchant acknowledged, preparing
    //   shipped → sent to customer
    //   cancelled → cancelled (stock is returned when cancelled from pending/confirmed)
    await sql`
