...
function renderThemedLanding(merchant, rawProducts, host) {
  ...
  // Prefer a real hosted product photo for the link preview; fall back to the
  // app icon (see kids-landing.js for why base64 images can't be used here).
  const firstRealPhoto = rawProducts.find((p) => p.image && /^https?:\/\//i.test(p.image));
...
<meta property="og:description" content="${esc(heroSubtitle)}">
...
const CONFIG = ${jsonScriptSafe(jsConfig)};
const SHIPPING_ZONES = ${jsonScriptSafe(DEFAULT_SHIPPING_ZONES)};
const WHATSAPP_NUMBER = ${jsonScriptSafe(waNumber)};
const STORE_NAME = ${jsonScriptSafe(storeName)};
const STORE_SLUG = ${jsonScriptSafe(merchant.slug)};
...
// Escape any text coming from the database (product name/description/image,
// attribute/variant values) before it's injected into innerHTML — prevents a
// malicious product name or custom attribute value from running as script in
  ...
  grid.innerHTML = list.map(p => {
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="card-attrs">\${attrBadges(p).map(b => '<span class="attr-badge">'+esc(b)+'</span>').join('')}</div>
        \${size ? \`<select class="size-select" id="sz-\${p.id}"><option value="">\${esc(CONFIG.sizeLabel)}</option>\${size.values.map(v => '<option value="'+esc(v)+'">'+esc(v)+'</option>').join('')}</select>\` : ''}
  ...
  wrap.innerHTML = cart.map(c => \`
    <div class="cart-item">
      ...
      <div class="cart-item-info">
        ...
        \${c.size ? '<div class="variant">'+esc(c.size)+'</div>' : ''}
        <div class="qty-ctrl">
...
async function submitOrder(){
  ...
  if (cart.length === 0 || !name || !phone || !addr || govIdx === ''){
    err.style.display = 'block'; err.textContent = 'من فضلك املأ كل البيانات';
    return;
  }
  ...
  const payload = {
    ...
    customer_governorate: gov.name,
    customer_address: addr,
    notes: (notes ? notes + ' | ' : '') + 'شحن ' + gov.name + ': ' + gov.price + ' ج.م',
    items: cart.map(c => ({ product_id: c.id, quantity: c.qty, variant_key: c.size || null })),
  };
  if (btn) { btn.disabled = true; btn.textContent = 'جاري إرسال طلبك...'; }
  try {
    const r = await fetch('/store/' + STORE_SLUG + '/order', {
      method:'POST',
    ...
    window.location.href = '/store/' + STORE_SLUG + '?ordered=' + (data.order_id||1);
  } catch (ex) {
    err.textContent = String(ex.message || ex);
    err.style.display = 'block';
    if (btn) { btn.disabled = false; btn.textContent = 'إتمام الشراء'; }
  }
}
