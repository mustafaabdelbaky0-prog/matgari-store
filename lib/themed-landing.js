...
function renderThemedLanding(merchant, rawProducts) {
  ...
  const products = rawProducts.map((p) => ({ ...p, attributes: coerceAttrs(p.attributes) }));
  ...
  const filters = buildFilters(products, cfg.filterKey);
...
function matchesFilter(p){
  if (activeFilter === '__all' || !CONFIG.filterKey) return true;
  const v = p.attributes ? p.attributes[CONFIG.filterKey] : null;
  if (!v) return false;
...
function sizeOptions(p){
  // Look for the most "size-like" multi attribute (sizes, volumes, models).
  const attrs = p.attributes || {};
  for (const a of CONFIG.attributes){
    if (a.type !== 'multi') continue;
    if (a.key === CONFIG.filterKey) continue;
    const v = attrs[a.key];
    if (Array.isArray(v) && v.length) return { key: a.key, label: a.label, values: v };
  }
  return null;
}
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        \${p.quantity <= 0
          ...
          : \`<button class="add-btn" id="add-\${p.id}" onclick="addToCart(\${p.id})">🛒 أضف للسلة</button>\`}
      </div>
    </div>\`;
  }).join('');
}
