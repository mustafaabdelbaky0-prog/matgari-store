...
function renderKidsLanding(merchant, products) {
...
const SHIPPING_ZONES = ${JSON.stringify(shippingZones)};
...
function renderProducts(){
  ...
  grid.innerHTML = PRODUCTS.map(p => \`
    <div class="card">
      ...
      <div class="card-body">
        \${p.quantity <= 0 ? '<div class="card-badge">نفدت الكمية</div>' : ''}
        <h3>\${p.name}</h3>
        ...
        \${p.quantity <= 0
          ? '<button class="add-btn" disabled style="background:#ccc;cursor:not-allowed">نفدت الكمية</button>'
          : \`<button class="add-btn" id="add-\${p.id}" onclick="addToCart(\${p.id})">🛒 أضيفي للسلة</button>\`}
      </div>
    </div>
  \`).join('');
}
...
function addToCart(pid){
  ...
  else cart.push({ id: p.id, name: p.name, image: p.image, price: p.price, qty: 1 });
  updateCart();
  const btn = document.getElementById('add-'+pid);
  if (btn){ btn.classList.add('added'); btn.textContent='✓ في السلة'; setTimeout(()=>{btn.classList.remove('added'); btn.textContent='🛒 أضيفي للسلة';}, 1200); }
}
...
function removeFromCart(pid){ cart = cart.filter(c => c.id !== pid); updateCart(); }
  ...
  wrap.innerHTML = cart.map(c => \`
    <div class="cart-item">
      ...
      <div class="cart-item-info">
        ...
        <button class="remove-btn" onclick="removeFromCart(\${c.id})">حذف</button>
      </div>
    </div>
  \`).join('');
...
function submitOrder(){
  ...
  const lines = [
    ...
    '',
    '🛒 الطلب:',
    ...cart.map(c => \`  • \${c.name} × \${c.qty} = \${money(c.qty * c.price)}\`),
    '',
